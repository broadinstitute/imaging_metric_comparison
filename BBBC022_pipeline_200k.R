...
filenames <- list.files(path = "../../input/BBBC022_2013/selected_single_cell_zoom/200000/feat_selected/") #BBBC022_dmso/Profile/feat_selected/SVD_entropy")
...
for(n in all){
  ...
  Pf <- readRDS(paste("../../input/BBBC022_2013/selected_single_cell_zoom/200000/feat_selected/", #Profile/feat_selected/SVD_entropy/", 
                    filenames[n], 
                    sep = ""))
Pf <- Pf$data %>%
...
dir.save <- "BBBC022_2013/selected_single_cell_zoom/200000" #/single_cells_findCorrelation"
...
data.filenames.p <- list.files(path = "../../input/BBBC022_2013/selected_single_cell_zoom/200000/hit_selected/Pearson/")
data.filenames.j <- list.files(path = "../../input/BBBC022_2013/selected_single_cell_zoom/200000/hit_selected/Jaccard") 
...
for(i in 1:length(data.filenames.p)){
  ...
  pf.p <- readRDS(file.path("..", "..", "input", "BBBC022_2013", "selected_single_cell_zoom", "200000","hit_selected", "Pearson", data.filenames.p[i]))
  enrichment.ratio <- bind_rows(enrichment.ratio, 
                                enrichment_ratio(pf.p, top.x = 0.02, seed = seed,
                                                 nCPU = detectCores(), N = 1000, filename = data.filenames.p[i], method = "Pearson"))
  
  
  pf.j <- readRDS(file.path("..", "..", "input", "BBBC022_2013", "selected_single_cell_zoom", "200000","hit_selected", "Jaccard", data.filenames.j[i]))  
  enrichment.ratio <- bind_rows(enrichment.ratio, 
                                enrichment_ratio(pf.j, top.x = 0.02, seed = seed, 
                                                 nCPU = detectCores(), N = 1000, filename = data.filenames.j[i], method = "Jaccard"))
}
...
filename.enr.ratio <- paste("../../input/BBBC022_2013/selected_single_cell_zoom/200000/enrichment_ratio/enr_ratio_nfeat_", 
                  as.character(numfeat),
                  #data.filenames.p[i], 
                  ".Rda",
                  sep = "")

enr.ratio[enr.ratio=="Jaccard"] <- "Jaccard_SVD_single_cell_200000"
enr.ratio[enr.ratio=="Pearson"] <- "Pearson_SVD_single_cell_200000"

enr.ratio %<>% mutate(n.feat = numfeat)#dim(Pf)[2])
