...
filenames <- list.files(path = "../../input/BBBC022_2013/selected_single_cell_zoom/feat_selected/")
...
for(n in all){
  ...
  Pf <- readRDS(paste("../../input/BBBC022_2013/selected_single_cell_zoom/feat_selected/", 
                      filenames[n], 
                      sep = ""))
  Pf <- Pf$data %>%
  ...
  for(i in 1:length(data.filenames.p)){
    ...
    pf.p <- readRDS(file.path("..", "..", "input", "BBBC022_2013", "selected_single_cell_zoom", "hit_selected", "Pearson", data.filenames.p[i]))
    ...
    pf.j <- readRDS(file.path("..", "..", "input", "BBBC022_2013", "selected_single_cell_zoom", "hit_selected", "Jaccard", data.filenames.j[i]))  
    enrichment.ratio <- bind_rows(enrichment.ratio, 
  ...
  filename.enr.ratio <- paste("../../input/BBBC022_2013/selected_single_cell_zoom/enrichment_ratio/enr_ratio_", 
                              data.filenames.p[i], 
                              ".Rda",
                              sep = "")
