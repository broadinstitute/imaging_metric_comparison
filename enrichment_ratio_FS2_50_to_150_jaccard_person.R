...
save(enrichment.ratio,file="enrichment_ratio_p.Rda")
...
save(enrichment.ratio,file="enrichment_ratio.Rda")



###### plot
enr.ratio <- enrichment.ratio %>% mutate(ratio = percent/mean)
enr.ratio %<>% mutate(n.feat = lapply(str_split(enr.ratio[,"filename"], "_"), function(x) x[4]) %>% unlist)
