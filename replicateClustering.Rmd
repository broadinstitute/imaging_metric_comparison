...
```{r import MOAs}
# import MOAs data
moa <- read.csv("../input/MOAs.csv") %>% mutate_if(is.factor, as.character) %>% plyr::rename(c("Name" = "Image_Metadata_SOURCE_COMPOUND_NAME")) 
moa$Image_Metadata_SOURCE_COMPOUND_NAME <- unlist(moa$Image_Metadata_SOURCE_COMPOUND_NAME)
# compounds name to upper case
moa$Image_Metadata_SOURCE_COMPOUND_NAME <- lapply(moa[, "Image_Metadata_SOURCE_COMPOUND_NAME"], stringr::str_to_upper)

# join moa data to cluster data 
sum.comp.clust <- dplyr::left_join(sum.comp.clust, moa, by = "Image_Metadata_SOURCE_COMPOUND_NAME")


knitr::kable(sum.comp.clust, caption = "Clustering of components with associated MOAs")
