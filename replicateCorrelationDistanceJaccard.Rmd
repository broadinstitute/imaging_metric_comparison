...
With features selection, computation time goes from ~15 minutes (~ 4 mins with parallelization) to ~5 minutes (~ 1.4 with parallelization).
...
comp.dist.median <- foreach(variable = 1:length(IDs$Image_Metadata_BROAD_ID), .combine=cbind) %dopar% {
...
```{r distance, include=FALSE, eval=FALSE}
start.time <- Sys.time()
...
| N = 3000  | 0.5875  |          |         |           |         |           |                           |                        |
| N = 4000  | 0.5950  |          |         |           |         |           |                           |                        |
| N = 5000  | 0.5944  | 0.5550   | 0.5668  | 0.4606    | 0.3612  | 0.4875    | ~ 0.6                     | 0.5906                 |
| N = 8000  | 0.5943  |          |         |           |         |           |                           | 0.5843                 |
| N = 10000 | 0.5925  |          |         |           |         |           |                           |                        |
...
| ------------------- | ------------------------------------- |
| without feat. sel.  | 0.6338   (nb of features in set = 50) |
| without feat. sel.  | 0.6581  (nb of features in set = 100) |
| with feat. sel.     | 0.6188   (nb of features in set = 50) |
| with feat. sel.     | 0.6300   (nb of features in set = 40) |
| with feat. sel.     | 0.6319   (nb of features in set = 30) |
| with feat. sel.     | 0.5863   (nb of features in set = 20) |
| with feat. sel.     | 0.5081   (nb of features in set = 10) |
...
saveRDS(pf, "../input/Pf_Gustafsdottir_hitSelection_distJ_featSel_30.rds")
